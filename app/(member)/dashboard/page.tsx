  ...
  useEffect(() => {
    async function fetchUserBookings() {
      try {
        ...
        const response = await fetch(`/api/bookings?userId=${user.id}&status=confirmed&upcoming=true`);
        ...
        const transformedBookings = data.bookings.map((booking: any) => ({
          id: booking.id,
          name: booking.class?.name || 'Class Name',
          instructor: booking.class?.instructor_name || 'Instructor',
          date: booking.booking_date + 'T' + booking.booking_time,
          duration: booking.class?.duration || 60,
        }));
        ...
        setUserBookings(upcomingClasses);
      } finally {
                ...
                <Card className="md:col-span-2">
                    ...
                    <div className="space-y-4">
                      {loadingBookings ? (
                        <div className="flex justify-center py-6">
                          <div className="w-8 h-8 border-2 border-ekantik-600 border-t-transparent rounded-full animate-spin"></div>
                        </div>
                      ) : userBookings.length > 0 ? (
                        userBookings.map((cls) => (
                          <div key={cls.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-800">
                        ...
                        <div className="text-center py-6">
                          ...
                          <Button variant="link" className="mt-2" asChild>
                            <Link href="/dashboard/classes">Browse classes</Link>
                          </Button>
                        </div>
                      )}
                    </div>
                  ...
                  <CardFooter>
                    <Button asChild className="w-full bg-ekantik-600 hover:bg-ekantik-700">
                      <Link href="/dashboard/classes">Book a New Class</Link>
                    </Button>
                  </CardFooter>
                </Card>
              </div>
